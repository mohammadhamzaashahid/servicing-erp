...
import RawMaterialForm from "@/modules/rawMaterials/RawMaterialForm";
import {
  useCreateRawMaterial,
  useDeleteRawMaterial,
  useRawMaterials,
  useUpdateRawMaterial,
} from "@/modules/rawMaterials/rawMaterial.hooks";
...
export default function RawMaterialsPage() {
  ...
  const rawMaterialsQuery = useRawMaterials(queryParams);
  ...
  const rows = rawMaterialsQuery.data?.data || [];
  const pagination = rawMaterialsQuery.data?.pagination;
  ...
  const openEditDrawer = (rawMaterial) => {
  ...
  const handleCreate = async (payload) => {
    ...
    try {
      ...
      setFormError(error.message || "Unable to create raw material");
    }
  };

  const handleUpdate = async (payload) => {
    if (!selectedRawMaterial?.id) return;
    ...
    try {
      ...
      setFormError(error.message || "Unable to update raw material");
    }
  };

  const handleDelete = async (rawMaterial) => {
    const confirmed = window.confirm(
      `Are you sure you want to delete raw material "${rawMaterial.name}"?`
    );

    if (!confirmed) return;

    try {
      await deleteMutation.mutateAsync(rawMaterial.id);
    } catch (error) {
      window.alert(
        error.message ||
          "Unable to delete raw material. If it has stock or transactions, mark it inactive instead."
      );
    }
  };
      ...
      render: (row) => (
        <div>
          ...
          <p className="text-xs text-slate-500">
            Unit: {row.unit?.code || "-"} {row.unit?.name ? `• ${row.unit.name}` : ""}
          </p>
        </div>
      ),
    },
  ...
  const drawerTitle =
    drawerMode === "create"
      ? "Create Raw Material"
      : `Edit ${selectedRawMaterial?.materialCode || ""}`;

  const drawerDescription =
    drawerMode === "create"
      ? "Add a new material that vendors supply and production consumes."
      : "Update raw material master details.";
  ...
  return (
    <div className="space-y-5">
      <section className="flex flex-col justify-between gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
            Manage materials purchased from vendors and consumed during
            manufacturing. Stock and average cost update automatically through
            transactions.
          </p>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm">
                {rawMaterialsQuery.error?.message || "Please try again."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-amber-200 bg-amber-50 p-5 text-amber-800">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm">
                You can view raw materials, but creating/editing requires units
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between px-1">
          <p className="text-sm text-slate-500">
            {pagination
              ? `${pagination.total} total raw material${
                  pagination.total === 1 ? "" : "s"
                }`
              : "Raw Materials"}
          </p>

          {rawMaterialsQuery.isFetching && !rawMaterialsQuery.isLoading ? (
        ...
        <DataTable
          columns={columns}
          rows={rows}
          loading={rawMaterialsQuery.isLoading}
          emptyTitle="No raw materials found"
          emptyDescription="Create raw materials before posting material receiving or production."
        />
      </section>
      ...
      >
        <RawMaterialForm
          mode={drawerMode}
          initialValues={selectedRawMaterial}
          units={units}
